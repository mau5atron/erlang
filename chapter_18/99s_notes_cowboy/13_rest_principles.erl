% REST Principles
	...
	% REST, you can read Roy T. Fielding's disstertation 
	% (https://www.ics.uci.edu/~fielding/pubs/dissertation/top.htm) as it
	% does a great job explaining where it comes from and what it achieves.
...
% REST Architecture
	...
	% of concerns allows both the client and server to evolve independently
	% as it only requires that the interface stays the same.
	...
	% authentication, then the client needs to authenticate itself with 
	% every request.

	% REST is "cacheable". The client, the server and any intermediary
	% components can all cache resourcesin order to improve performance.
	...
	% REST is a "layered system". Individual components cannot see  beyond
	% the immediate layer with which they are interacting. this means that a
	% client connecting to an intermediate component, like a proxy, has no
	% knowledge of what lies beyond. This allows components to be indpendent
...
% Resources and Resource Identifiers
	...
	% A resource is a conceptual mapping to a set of entities. This set of
	...
	% then the first resource may map to the empty set.
	% This resource exists regardless of the information it maps to.
	...
	% also be used, but these can be directly translated int oa URI.
...
% Resource Representations
	...
	% This metadata comes as a list of key-value pairs, where the name
	...
	% response headers. The headers' structure and semantics are well
	...
	% Resource metadata could be linked to related resources or information
	% about additional representations of the resource.

	% Control data allows parameteriing the request or response. For
	% example, we may only want the representation returned if it is more
	% recent that the one we have in cache. Similarly, we may want to
	% instruct the client about how it should cache the representation.
	% This isn't restricted to caching. We may, for example, want to store
	% a new representation of a resource only if it wasn't modified since
	% we first retrieved it.
...
% Self-Descriptive Messages
	...
	% representation returned). If you are sending HTML, then one must say it
	...
	% Ths means that you can create your own media types, like
	% "application/x-mine", and that as long as you write the specifications
	% for it and that both endpoints agree about it then the constraint
	% is respected.
...
% Hypermedia as the Engine of Application State
	...
	% RESTful. This is the case for the majority of self-proclaime REST
	% services.

	% On the other hand if you create a JSON based media type that has a
	% concept of links and link relations, then your service might be
	% RESTful.

	% Respecting this constraint means that the entirety of the service
	% becomes self-discoverable, not only the resources on it, but also the
