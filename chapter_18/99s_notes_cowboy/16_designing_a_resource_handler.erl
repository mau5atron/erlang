...
% The Service
	...
	% Can the service become unavailable, and when it does, can we detect it?
		...
		% may also have planned outages of all or parts of the system.
		% Implement the "service_unavailable" callback.
...
% Collection Handler
	...
	% hardcodedl if you use "/forums/:cateogory" for the collection of
	% threads then it isn't. When the collection is hardcoded you can safely
	% assume the resource always exists.

	% When methods should I implement?

	% OPTIONS is used to get somee information about the collection. It is
	...
	% be created before known their URI, usually because parts of it are
	...
	% usedd for such things as reordering; adding, modifying or deleting
...
% Single Resource Handler
	...
	% PATCH i used to modify the resource using instructions given in the
	% request body. A PATCH operation is atomic. The PATCH operation may be
...
% The Resource
	...
	% Following the above discussion, implement the "allowed_method"
	% callback.
	...
	% resource? What authentication mechansisms should I provide? This may
	...
	% Do I need find-grained access control? How do I determine that they are
	...
	% Can access be rate-limited or authenticated users? Use the
	% "rate_limited" callback.

	% Are there any constraints on the length of the resource URI? For
	% example, the URI may be used as a key in storage and may have a limit
	% in length. Implement "uri_too_long".
...
% Representations
	...
	% Implement the mandatory "content_types_provided". Prefixthe callbacks
	% with "to_" for clarity. For example, "to_html" or "to_text". For
	% resources that don't implement methods GET or HEAD, you must still
	% accept at least one media type, but you can leave the callback as
	% "undefined" since it will never be called.
...
% Redirections
	...
	% Do I keep track of what resources were deleted? For example, you may
	% have a mechanism where moving a resources leaves a redirect link to its
	% new location. Implement the "previously_existed" callback.

	% Was the resource moved, as is the move temporary? If it is explicitly
...
% The Request
	...
	% "malformed_request" and do all the parsing and validation in this
	% function. Note that the body should not be read at this point.

	% May there be a request body? Will I know its size? What's the maxiumum
	% size of the request body I'm willing to accept? Implement
	% "valid_entity_length".

	% Finally, take a long at the sections corresponding to the methods you
	% are implementing.
...
% OPTIONS Method
	...
	% to add more information to the response? Implement the "options"
	% method.
...
% PUT, POST and PATCH Methods
	...
	% the "content_types_accepted" callback, and one "AcceptCallback"
	% callback for each content-type it returns. Prefix the "AcceptCallback"
...
% DELETE Methods
	...
	% the server, including form any caching service? If not, and/or if the
