% Rest Flowcharts
	...
	% All paths start with the "Start" diagram, and all paths exluding the
	...
	% default behavior if the callback is undefined. The star next to values
	% indicate that the value is descriptive rather than exact.
...
% Start
	...
	% of these steps. It is only processed at the end of the "PUT", "POST",
	% and "PATCH" methods diagram, when all conditions have been met.
...
% Content Negotiation
	...
	% executed right after the "Start" diagram is completed.
		...
		% Look at rest_content_negotiation.png
	...
	% of provided content-type, language or charset for this resource.
	...
	% values in the Req object. You shouls use the appropriate representation
	% if you set a response body manually (alongside an error code, for
	% example).
...
% GET and HEAD methods
	...
	% The following diagram only applies to GET and HEAD requests.

	% For a description of the "cond" step, please see the "Conditional
	% requests" diagram.

		% Look at rest_get_head_conditional_request.png
...
% PUT, POST and PATCH Methods
	...
	% requests" diagram.
		...
		% Look at rest_put_post_patch.png

	% When the resource exists, fitst the conditiona steps are executed. When
	% that succeeds, and the method is PUT, Cowboy will call the
	...
	% step. For other methods, Cowboy will figure out whether he resource
	...
	% The "moved_permanently" and "moved_temporarily" callbacks must return
	% the new location of the reosurce if it was in fact moved.
	...
	% location headerr or a body in the response.
...
% DELETE Method
	...
	% requests" diagram.
		...
		% Look at "rest_delete.png"

	% When the resource exists, and the conditional steps succeed, the
	% resource can be deleted.
	
	% Deleting the resource is a two step process. First the callback
	% "delete_resource" is executed. Use this callback to delete the
	% resource.

	% Because the resource mayw be cached, you must also delete all cached
...
% Conditional Requests
	...
	% exists.
		...
		% Look at rest_conditional_requests.png
	...
	% retrival or modification of the resource.
