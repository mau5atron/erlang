...
% Sequential and Parallel Servers
	...
	% In the previous section, we made a server that accepted only one
	% connection and then terminated. By changing this code slightly, we can
	% make two different types of server

		% * A sequential server - one that accepts one connection at a time
		% * A parallel server - one that accepts multiple parallel connections
		% at the same time
...
% A Sequential Server
	...
	% to serve more than one request, we leave the listening socket open and 
	% don't call gen_tcp:close(Listen). The other difference is that after 
	% loop(Socket) has finished, we call seq_loop(Listen) again, which waits 
	% for the next connection.

	% If a client tries to connect to the server while the server is busy 
	% with an existing connection, then the connectionwill be queued until 
	% the server has finished with the existing connection. If the number of 
	% queued connections exceeds the listen backlog, then the connection 
	% will be rejected.
